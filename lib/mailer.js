...
export const formatVerificationBody = (
  ...
  const header = 'before proceeding we need to verify your email\n\n';
  const introduction = `according to our records you're requestig a pair of ${brand} ${model} in size ${size}\n`;
  const body = `if this sounds right please click in the link below:\n`;
  const footer = '\n\nthanks for your patiance! with some luck the owner will get in touch soon.';

  return header + introduction + body + verificationURL + footer;
};
